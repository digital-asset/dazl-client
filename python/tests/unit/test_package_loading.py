...
@pytest.mark.asyncio
@pytest.mark.skip(
    "This test is no longer valid, as we do not force packages to be loaded on startup"
)
async def test_package_loading(sandbox):
    ...
    logging.info(
        "Package IDs (%s total): %s", len(expected_package_ids), sorted(expected_package_ids)
    )
