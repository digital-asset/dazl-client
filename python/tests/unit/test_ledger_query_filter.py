...
from dazl.query import EMPTY, Filter, Queries, parse_query
import pytest
...
def test_empty_query(server_side_filters: bool) -> None:
    actual = parse_query(server_side_filters=server_side_filters)
    assert actual is None


@pytest.mark.parametrize("server_side_filters", [False, True])
def test_single_template(server_side_filters: bool) -> None:
...
@pytest.mark.parametrize("server_side_filters", [False, True])
def test_two_templates(queries: Sequence[Queries], server_side_filters: bool) -> None:
...
def test_two_templates_as_dict_with_one_match_server_side_filters() -> None:
...
def test_two_templates_as_dict_with_one_match_no_server_side_filters() -> None:
    """
