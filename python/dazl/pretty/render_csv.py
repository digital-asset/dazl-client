...
from dazl.model.types import Type, TypeReference, RecordType, ListType, ContractIdType
from dazl.model.types_store import PackageStore


def write_csv(store: PackageStore) -> str:
    templates = {template.data_type.name.full_name: template
                 for template in store.resolve_template('*')}
