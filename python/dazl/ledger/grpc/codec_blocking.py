...
class Codec:
    ...
    def encode_command(self, cmd: Command) -> G_Command:
        if isinstance(cmd, CreateCommand):
            return G_Command(create=self.encode_create_command(cmd.template_id, cmd.payload))
    ...
    def encode_filters(self, template_ids: Sequence[TypeConName]) -> G_Filters:
    ...
    def decode_created_event(self, event: G_CreatedEvent) -> CreateEvent:
    ...
    def decode_archived_event(self, event: G_ArchivedEvent) -> ArchiveEvent:
        cid = self.decode_contract_id(event)
        return ArchiveEvent(cid)

    def decode_exercise_response(self, tree: G_TransactionTree) -> ExerciseResponse:
        """
