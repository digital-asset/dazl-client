...
from ...ledger.blocking import PackageService
...
from ..blocking import PackageLoader
...
class Codec:
    ...
    def encode_command(self, cmd: Command) -> lapipb.Command:
        if isinstance(cmd, CreateCommand):
            return lapipb.Command(create=self.encode_create_command(cmd.template_id, cmd.payload))
        elif isinstance(cmd, ExerciseCommand):
            return lapipb.Command(
                exercise=self.encode_exercise_command(cmd.contract_id, cmd.choice, cmd.argument)
            )
        elif isinstance(cmd, ExerciseByKeyCommand):
            return lapipb.Command(
                exerciseByKey=self.encode_exercise_by_key_command(
                    cmd.template_id, cmd.choice, cmd.key, cmd.argument
                )
            )
        elif isinstance(cmd, CreateAndExerciseCommand):
            return lapipb.Command(
                createAndExercise=self.encode_create_and_exercise_command(
    ...
    def encode_create_command(
        self, template_id: Union[str, Any], payload: ContractData
    ) -> lapipb.CreateCommand:
        item_type = self._loader.do_with_retry(lambda: self._lookup.template_name(template_id))
    ...
    def encode_exercise_command(
        self,
        contract_id: ContractId,
        choice_name: str,
        argument: Optional[Any] = None,
    ) -> lapipb.ExerciseCommand:
        item_type, _, choice = self._look_up_choice(contract_id.value_type, choice_name)
        ...
        value_field, value_pb = self.encode_value(choice.arg_binder.type, argument)
        set_value(cmd_pb.choice_argument, value_field, value_pb)

        return cmd_pb

    def encode_create_and_exercise_command(
        self,
        ...
        item_type, _, choice = self._look_up_choice(template_id, choice_name)

        payload_field, payload_pb = self.encode_value(con(item_type), payload)
        if payload_pb != "record":
            raise ValueError("unexpected non-record type when constructing payload")
        argument_field, argument_pb = self.encode_value(choice.arg_binder.type, argument)
    ...
    def encode_exercise_by_key_command(
        self,
        template_id: Union[str, TypeConName],
        choice_name: str,
        key: Any,
        ...
        item_type, template, choice = self._look_up_choice(template_id, choice_name)
        ...
        key_field, key_pb = self.encode_value(template.key.type, key)
        value_field, value_pb = self.encode_value(choice.arg_binder.type, argument)
    ...
    def encode_filters(self, template_ids: Sequence[TypeConName]) -> lapipb.Filters:
        ...
        for template_id in template_ids:
            requested_types.update(
                self._loader.do_with_retry(lambda: self._lookup.template_names(template_id))
            )
    ...
    def encode_value(self, item_type: Type, obj: Any) -> Tuple[str, Optional[Any]]:
        """
        Convert a dazl/Python value to its Protobuf equivalent.
        """
        return self._loader.do_with_retry(lambda: self._encode_context.convert(item_type, obj))
    ...
    def decode_created_event(self, event: lapipb.CreatedEvent) -> CreateEvent:
        cid = self.decode_contract_id(event)
        cdata = self.decode_value(con(cid.value_type), event.create_arguments)
        ...
        if template is not None and template.key is not None:
            key = self.decode_value(template.key.type, event.contract_key)
    ...
    def decode_archived_event(self, event: lapipb.ArchivedEvent) -> ArchiveEvent:
        cid = self.decode_contract_id(event)
        return ArchiveEvent(cid)

    def decode_exercise_response(self, tree: lapipb.TransactionTree) -> ExerciseResponse:
        """
        ...
        for event_id in tree.root_event_ids:
            ...
            if event_pb_type == "created":
                events.append(self.decode_created_event(event_pb.created))
                ...
                events.extend(self._decode_exercised_child_events(tree, [event_id]))
            else:
                LOG.warning("Received an unknown event type: %s", event_pb_type)

        return ExerciseResponse(result, events)

    def _decode_exercised_child_events(
        ...
        for event_id in event_ids:
            ...
            if event_pb_type == "created":
                ...
                events.extend(
                    self._decode_exercised_child_events(tree, event_pb.exercised.child_event_ids)
                )
            else:
                LOG.warning("Received an unknown event type: %s", event_pb_type)
        return events

    def decode_value(self, item_type: Type, obj: Any) -> Optional[Any]:
        """
        Convert a Protobuf Ledger API value to its dazl/Python equivalent.
        """
        return self._loader.do_with_retry(lambda: self._decode_context.convert(item_type, obj))
    ...
    def _look_up_choice(
        ...
        template_type = self._loader.do_with_retry(lambda: self._lookup.template_name(template_id))
