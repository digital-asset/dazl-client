...
class Connection(blocking.Connection):
    ...
    def open(self) -> None:
        ...
        if not self._config.access.ledger_id:
            ...
            response = stub.GetLedgerIdentity(lapipb.GetLedgerIdentityRequest())
    ...
    def create(
        ...
        *,
        ...
        commands = [
            lapipb.Command(create=self._codec.encode_create_command(__template_id, __payload))
        ]
        request = lapipb.SubmitAndWaitRequest(
            commands=lapipb.Commands(
                ledger_id=self._config.access.ledger_id,
...
class QueryStream(blocking.QueryStreamBase):
    ...
    ) -> Iterable[Union[CreateEvent, Boundary]]:
