...
from ..prim import to_bool, to_str, datetime_to_str, date_to_str, to_decimal, decimal_to_str, to_int, to_party, \
    to_variant
...
class StringMapperBase(ValueMapper):
    ...
    def data_enum(
            self, context: 'Context', dt: 'DefDataType', enum: 'DefDataType.EnumConstructors',
            obj: 'Any') -> 'Any':
    ...
    def prim_gen_map(self, context: 'Context', key_type: 'Type', value_type: 'Type', obj: 'Any') \
            -> 'Any':
        return "{" + ", ".join(
            key + ": " + value
            for key, value in context.convert_gen_map(
                key_type, value_type, obj, mapper=FLAT).items()) + "}"
