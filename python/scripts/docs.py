...
def main(argv: "Sequence[str]"):
    ...
    argp.add_argument(
        "--output-path",
        "-o",
        default=os.getenv("DAZL_DOCS_OUT", str(DAZL_ROOT / "dist" / "documentation")),
    )
    ...
    if cmd == "build":
        if not args.output_path:
            print(
                "Either the --output-path command-line parameter or the DAZL_DOCS_OUT variable are required"
            )
            sys.exit(2)
        if not args.dry_run:
            build_docs(dba)
...
def parse_arguments(
    output_path: "Path", output_format: "str", theme_path: "Optional[Path]"
) -> "DocumentationBuildArguments":
