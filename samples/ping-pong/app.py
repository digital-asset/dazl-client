...
def send(party, cid, choice_name, count):
    logging.info(party + ' is exercising ' + choice_name + ' on ' + str(cid) + ' at count ' + str(count))
...
def init_pingpong(network: dazl.Network, sender, receiver):
    ...
    async def init(event: ReadyEvent):
        pings = event.acs_find_active('PingPong.Ping', match=lambda cdata: cdata['sender'] == sender)
        pongs = event.acs_find_active('PingPong.Pong', match=lambda cdata: cdata['receiver'] == sender)
        if len(pings) == 0 and len(pongs) == 0:
            logging.info("Initializing Ping for " + sender)
            return dazl.create('PingPong.Ping', {'sender': sender, 'receiver': receiver, 'count': 0})
...
def register_pingpong_handler(network: dazl.Network, party):
    ...
    async def init(event: ReadyEvent):
        ...
        pings = event.acs_find_active('PingPong.Ping', match=lambda cdata: cdata['receiver'] == party)
        for pingCid, pingData in pings.items():
            cmds.append(send(party, pingCid, 'RespondPong', pingData['count']))

        pongs = event.acs_find_active('PingPong.Pong', match=lambda cdata: cdata['receiver'] == party)
