...
class PartyBot:
    ...
    def send_to_five_friends(self, event):
        if self.party_name == event.cdata["author"]:
            ...
            choices = []
            for receiver_index in map(
                lambda i: i % MEMBER_PARTY_COUNT, range(party_index + 1, party_index + 6)
            ):
                ...
                choices.append(
                    (
                        ...
                        dict(
                            ...
                            content="I am a letter from {} to {}".format(
                                self.party_name, receiver_address
                            ),
                        ),
                    )
                )
                choices.append(
                    (
                        "CreateTimeLetter",
                        dict(address=receiver_address, content=datetime(2000, 1, 1)),
                    )
                )
                choices.append(
                    ("CreateListIntLetter", dict(address=receiver_address, content=[0, 1, 2, 3, 4]))
                )

            return [
                ExerciseCommand(event.cid, choice_name, choice_args)
...
class PostOffice:
    ...
    def register_address(self, event):
        ...
        if unknown_letters:
            return [
                letter_cid.exercise("Sort", dict(receiverCid=event.cid))
...
def run_test(url, keepalive=False):
    ...
    postman_client.add_ledger_ready(
        lambda event: CreateCommand("Main:PostmanRole", dict(postman=POSTMAN_PARTY))
    )
    postman_client.add_ledger_created(
        "Main:PostmanRole",
        lambda event: [ExerciseCommand(event.cid, "InviteParticipant", m) for m in members],
    )
    ...
    postman_client.add_ledger_created(
        "Main:SortedLetter", lambda event: ExerciseCommand(event.cid, "Deliver")
    )
    ...
    for member_client in member_clients:
        ...
        member_client.add_ledger_created(
            "Main:InviteAuthorRole",
            lambda event: ExerciseCommand(event.cid, "AcceptInviteAuthorRole"),
        )
        member_client.add_ledger_created(
            "Main:InviteReceiverRole",
            lambda event: ExerciseCommand(event.cid, "AcceptInviteReceiverRole"),
        )
    ...
    LOG.info(
        "%s letters were received before the recipients were known.",
        post_office._unsorted_letter_count,
    )
...
def _main():
    ...
    backend_args.add_argument(
        "--url", required=False, help="The URL of the *existing* server to connect to"
    )
    backend_args.add_argument(
        "--port", required=False, type=int, help="The port to spin up a NEW Sandbox"
    )
