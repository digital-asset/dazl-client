...
class PartyBot:
    ...
    def send_to_five_friends(self, event):
        if self.party_name == event.cdata["author"]:
            ...
            return [
                exercise(event.cid, choice_name, choice_args)
...
def run_test(url, keepalive=False):
    ...
    postman_client.add_ledger_ready(
        lambda event: create("Main:PostmanRole", dict(postman=POSTMAN_PARTY))
    )
    postman_client.add_ledger_created(
        "Main:PostmanRole",
        lambda event: [exercise(event.cid, "InviteParticipant", m) for m in members],
    )
    ...
    postman_client.add_ledger_created(
        "Main:SortedLetter", lambda event: exercise(event.cid, "Deliver")
    )
    ...
    for member_client in member_clients:
        ...
        member_client.add_ledger_created(
            "Main:InviteAuthorRole", lambda event: exercise(event.cid, "AcceptInviteAuthorRole")
        )
        member_client.add_ledger_created(
            "Main:InviteReceiverRole", lambda event: exercise(event.cid, "AcceptInviteReceiverRole")
        )
